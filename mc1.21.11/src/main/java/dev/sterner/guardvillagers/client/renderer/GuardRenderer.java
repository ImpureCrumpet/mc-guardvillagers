...
public class GuardRenderer extends BipedEntityRenderer<GuardEntity, GuardBipedRenderState, BipedEntityModel<GuardBipedRenderState>> {
    ...
    public GuardRenderer(EntityRendererFactory.Context context) {
        ...
        EquipmentModelData<BipedEntityModel<GuardBipedRenderState>> playerArmor = EquipmentModelData.mapToEntityModel(
                EntityModelLayers.PLAYER_EQUIPMENT,
                context.getEntityModels(),
                BipedEntityModel::new
        );
        BipedEntityModel<GuardBipedRenderState> guardInner = new GuardArmorModel(context.getPart(GuardVillagersClient.GUARD_ARMOR_INNER));
        BipedEntityModel<GuardBipedRenderState> guardOuter = new GuardArmorModel(context.getPart(GuardVillagersClient.GUARD_ARMOR_OUTER));
        EquipmentModelData<BipedEntityModel<GuardBipedRenderState>> guardInnerData = new EquipmentModelData<>(guardInner, guardInner, guardInner, guardInner);
        EquipmentModelData<BipedEntityModel<GuardBipedRenderState>> guardOuterData = new EquipmentModelData<>(guardOuter, guardOuter, guardOuter, guardOuter);

        this.addFeature(new ArmorFeatureRenderer<>(
                this,
                GuardVillagersConfig.useSteveModel ? playerArmor : guardInnerData,
                GuardVillagersConfig.useSteveModel ? playerArmor : guardOuterData,
        ...
        if (!held.isEmpty()) {
            ...
            if (entity.getItemUseTimeLeft() > 0) {
                ...
                switch (useAction) {
                    ...
                    case SPEAR -> pose = BipedEntityModel.ArmPose.SPEAR;
                    case CROSSBOW -> {
