...
public class GuardEntity extends PathAwareEntity implements CrossbowUser, RangedAttackMob, Angerable, InventoryChangedListener, InteractionObserver {
    ...
    protected void readCustomData(ReadView view) {
        ...
        view.read("Owner", Uuids.CODEC).ifPresent(uuid -> {
            try {
        ...
        view.read("Gossips", VillagerGossips.CODEC).ifPresent(restored -> {
            this.gossips.clear();
            this.gossips.shareGossipFrom(restored, this.random, Integer.MAX_VALUE);
        });
        for (ReadView entry : view.getListReadView("Inventory")) {
            int slot = entry.getByte("Slot", (byte) 0) & 255;
            entry.read(ItemStack.MAP_CODEC).ifPresent(stack -> this.guardInventory.setStack(slot, stack));
        }
        int armorIndex = 0;
        for (ItemStack stack : view.getTypedListView("ArmorItems", ItemStack.CODEC)) {
            if (!stack.isEmpty()) {
        ...
        int handIndex = 0;
        for (ItemStack stack : view.getTypedListView("HandItems", ItemStack.CODEC)) {
            if (handIndex >= 2) {
                break;
            }
            int handSlot = handIndex == 0 ? 5 : 4;
            if (!stack.isEmpty()) {
        ...
        if (!getEntityWorld().isClient()) {
            this.readAngerFromData(getEntityWorld(), view);
        }
    }
    ...
    protected void writeCustomData(WriteView view) {
        ...
        if (this.getOwnerId() != null) {
            view.put("Owner", Uuids.CODEC, this.getOwnerId());
        }

        WriteView.ListView inventory = view.getList("Inventory");
        for (int i = 0; i < this.guardInventory.size(); ++i) {
            ItemStack itemstack = this.guardInventory.getStack(i);
            if (!itemstack.isEmpty()) {
                WriteView entry = inventory.add();
                entry.putByte("Slot", (byte) i);
                entry.put(ItemStack.MAP_CODEC, itemstack);
            }
        }
    ...
    public LivingEntity getOwner() {
        try {
            ...
            boolean heroOfTheVillage = uuid != null && getEntityWorld().getPlayerAnyDimension(uuid) != null && getEntityWorld().getPlayerAnyDimension(uuid).hasStatusEffect(StatusEffects.HERO_OF_THE_VILLAGE);
            return uuid == null || (getEntityWorld().getPlayerAnyDimension(uuid) != null && (!heroOfTheVillage && GuardVillagersConfig.followHero) || !GuardVillagersConfig.followHero && getEntityWorld().getPlayerAnyDimension(uuid) == null) ? null : getEntityWorld().getPlayerAnyDimension(uuid);
    ...
    public void onDeath(DamageSource damageSource) {
        if ((getEntityWorld().getDifficulty() == Difficulty.NORMAL || getEntityWorld().getDifficulty() == Difficulty.HARD) && damageSource.getAttacker() instanceof ZombieEntity) {
            ZombieVillagerEntity zombieguard = (ZombieVillagerEntity) this.convertTo(EntityType.ZOMBIE_VILLAGER, EntityConversionContext.create(this, true, false), zombie -> {
            });
            if (getEntityWorld().getDifficulty() != Difficulty.HARD && this.random.nextBoolean() || zombieguard == null) {
                return;
            }
            zombieguard.initialize((ServerWorldAccess) getEntityWorld(), ((ServerWorld) getEntityWorld()).getLocalDifficulty(zombieguard.getBlockPos()), SpawnReason.CONVERSION, new ZombieEntity.ZombieData(false, true));
        ...
        if (this.activeItemStack.getItem() == Items.SHIELD) { // Might create compatibility problems with other mods that add shields
            if (amount >= 3.0F) {
                ...
                if (this.activeItemStack.isEmpty()) {
                    ...
                    this.playSound(SoundEvents.ITEM_SHIELD_BREAK.value(), 0.8F, 0.8F + getRandom().nextFloat() * 0.4F);
                }
            }
        }
    }
    ...
    protected void initGoals() {
        ...
        if (GuardVillagersConfig.attackAllMobs)
            this.targetSelector.add(3, new ActiveTargetGoal<>(this, MobEntity.class, 5, true, true, (mob, serverWorld) -> mob instanceof Monster && !GuardVillagersConfig.mobBlackList.contains(mob.getSavedEntityId())));
        ...
        switch (slotIn) {
            case CHEST:
                if (this.guardInventory.getStack(1).isEmpty())
                    this.guardInventory.setStack(1, this.getEquippedStack(slotIn));
                break;
            case FEET:
                if (this.guardInventory.getStack(3).isEmpty())
                    this.guardInventory.setStack(3, this.getEquippedStack(slotIn));
                break;
            case HEAD:
                if (this.guardInventory.getStack(0).isEmpty())
                    this.guardInventory.setStack(0, this.getEquippedStack(slotIn));
                break;
            case LEGS:
                if (this.guardInventory.getStack(2).isEmpty())
                    this.guardInventory.setStack(2, this.getEquippedStack(slotIn));
                break;
            case MAINHAND:
                this.guardInventory.setStack(5, this.getEquippedStack(slotIn));
                break;
            case OFFHAND:
                this.guardInventory.setStack(4, this.getEquippedStack(slotIn));
                break;
        }
    }
        ...
        if (damage >= 0.0F) {
            ...
            for (int i = 0; i < this.guardInventory.size(); ++i) {
                ...
                if (equippable != null) {
                    ...
                    if (damageSource.isOf(DamageTypes.ON_FIRE) && resist != null && resist.resists(damageSource)) {
                        continue;
                    }
                    int j = i;
    ...
    public void onStruckByLightning(ServerWorld world, LightningEntity lightning) {
        if (world.getDifficulty() != Difficulty.PEACEFUL) {
            ...
            witchentity.initialize(world, ((ServerWorld) world).getLocalDifficulty(witchentity.getBlockPos()), SpawnReason.CONVERSION, null);
    ...
    public void chooseRandomAngerTime() {
        this.setAngerEndTime(this.getEntityWorld().getTime() + angerTime.get(this.random));
    }
