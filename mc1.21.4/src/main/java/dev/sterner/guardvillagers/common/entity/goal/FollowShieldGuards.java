...
public class FollowShieldGuards extends Goal {
    ...
    public boolean canStart() {
        List<? extends GuardEntity> list = this.taskOwner.getWorld().getEntitiesByClass(GuardEntity.class, this.taskOwner.getBoundingBox().expand(8.0D, 8.0D, 8.0D), guard -> true);
        if (!list.isEmpty()) {
            for (GuardEntity guard : list) {
                if (!guard.isInvisible() && guard.getOffHandStack().getItem() == Items.SHIELD && guard.isBlocking()
                        && this.taskOwner.getWorld().getEntitiesByClass(
                        GuardEntity.class,
