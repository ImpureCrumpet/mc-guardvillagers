...
public class GuardVillagerScreen extends HandledScreen<GuardVillagerScreenHandler> {
    ...
    protected void drawBackground(DrawContext ctx, float delta, int mouseX, int mouseY) {
        ...
        ctx.drawTexture(RenderLayer::getGuiTextured, GUARD_GUI_TEXTURES, i, j, 0, 0, this.backgroundWidth, this.backgroundHeight, 256, 256);
    ...
    private void drawHeart(DrawContext context, HeartType type, int x, int y, boolean half) {
        ...
        context.drawGuiTexture(RenderLayer::getGuiTextured, type.getTexture(half), x, y, 9, 9);
    ...
    protected void drawForeground(DrawContext ctx, int x, int y) {
        ...
        //Armor
        for (int i = 0; i < 10; i++) {
            ctx.drawGuiTexture(RenderLayer::getGuiTextured, ARMOR_EMPTY_TEXTURE, (i * 8) + 80, 30, 9, 9);
        }
        for (int i = 0; i < armor / 2; i++) {
            if (armor % 2 != 0 && armor / 2 == i + 1) {
                ctx.drawGuiTexture(RenderLayer::getGuiTextured, ARMOR_FULL_TEXTURE, (i * 8) + 80, 30, 9, 9);
                ctx.drawGuiTexture(RenderLayer::getGuiTextured, ARMOR_HALF_TEXTURE, ((i + 1) * 8) + 80, 30, 9, 9);
            } else {
                ctx.drawGuiTexture(RenderLayer::getGuiTextured, ARMOR_FULL_TEXTURE, (i * 8) + 80, 30, 9, 9);
            }
        }
    ...
    class GuardGuiButton extends TexturedButtonWidget {
        ...
        public void renderWidget(DrawContext graphics, int mouseX, int mouseY, float partialTicks) {
            ...
            graphics.drawGuiTexture(RenderLayer::getGuiTextured, resourcelocation, this.getX(), this.getY(), this.width, this.height);
        }
    }
