...
public class HealGuardAndPlayerGoal extends Goal {
    ...
    public boolean canStart() {
        ...
        if (!list.isEmpty()) {
            for (LivingEntity mob : list) {
                if (mob != null) {
                    if (mob instanceof VillagerEntity && mob.isAlive() && mob.getHealth() < mob.getMaxHealth() && mob != healer || mob.getType() == GuardVillagers.GUARD_VILLAGER && mob.isAlive() && mob.getHealth() < mob.getMaxHealth()
    ...
    public void throwPotion(LivingEntity target, float distanceFactor) {
        ...
        var potion = target.getHealth() <= 4.0F ? Potions.HEALING : Potions.REGENERATION;
        ItemStack stack = PotionContentsComponent.createStack(Items.SPLASH_POTION, potion);
        if (healer.getWorld() instanceof ServerWorld serverWorld) {
            SplashPotionEntity potionentity = new SplashPotionEntity(serverWorld, healer, stack);
