...
public class GuardEntity extends PathAwareEntity implements CrossbowUser, RangedAttackMob, Angerable, InventoryChangedListener, InteractionObserver {
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        nbt.get("Owner", Uuids.CODEC).ifPresent(uuid -> {
            try {
        ...
        this.lastGossipTime = nbt.getLong("LastGossipTime", 0L);
        ...
        nbt.get("Gossips", VillagerGossips.CODEC).ifPresent(restored -> {
            this.gossips.clear();
            this.gossips.shareGossipFrom(restored, this.random, Integer.MAX_VALUE);
        });
        NbtList listnbt = nbt.getListOrEmpty("Inventory");
        for (int i = 0; i < listnbt.size(); ++i) {
            NbtCompound nbtnbt = listnbt.getCompoundOrEmpty(i);
            int j = nbtnbt.getByte("Slot", (byte) 0) & 255;
            ItemStack.fromNbt(this.getRegistryManager(), nbtnbt).ifPresent(stack -> this.guardInventory.setStack(j, stack));
        }
        if (nbt.contains("ArmorItems")) {
            NbtList armorItems = nbt.getListOrEmpty("ArmorItems");
            for (int i = 0; i < armorItems.size() && i < 4; ++i) {
                ItemStack stack = ItemStack.fromNbt(this.getRegistryManager(), armorItems.getCompoundOrEmpty(i)).orElse(ItemStack.EMPTY);
                if (!stack.isEmpty()) {
        ...
        if (nbt.contains("HandItems")) {
            NbtList handItems = nbt.getListOrEmpty("HandItems");
            for (int i = 0; i < handItems.size(); ++i) {
                int handSlot = i == 0 ? 5 : 4;
                ItemStack stack = ItemStack.fromNbt(this.getRegistryManager(), handItems.getCompoundOrEmpty(i)).orElse(ItemStack.EMPTY);
                if (!stack.isEmpty()) {
    ...
    public void writeCustomDataToNbt(NbtCompound nbt) {
        ...
        if (this.getOwnerId() != null) {
            nbt.put("Owner", Uuids.CODEC, this.getOwnerId());
        }
        ...
        for (int i = 0; i < this.guardInventory.size(); ++i) {
            ...
            if (!itemstack.isEmpty()) {
                NbtCompound nbtnbt = (NbtCompound) itemstack.toNbt(this.getRegistryManager());
        ...
        if (this.activeItemStack.getItem() == Items.SHIELD) { // Might create compatibility problems with other mods that add shields
            if (amount >= 3.0F) {
                ...
                if (this.activeItemStack.isEmpty()) {
                    ...
                    this.playSound(SoundEvents.ITEM_SHIELD_BREAK.value(), 0.8F, 0.8F + getWorld().random.nextFloat() * 0.4F);
                }
            }
        }
    }
